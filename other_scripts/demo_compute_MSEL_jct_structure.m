...
%     img = repmat(img, [1,1,3]);
    ...
    tic;
    [new_cfrags, new_cfrags_idx, corner_pts, junction_pts] = contour_breaker_geom_v2(CEM{2}, cfrags_idx, edgemap, params);
    toc;
...
%     tic
%     [G, merged_cem, merged_cf_idx] = merge_cfrags_graphical_model_v2(new_cfrags, new_cfrags_idx, edges, hsv_img, tmap, edgemap, params);
%     toc
...
%         p = len;
    ...
    set(H, 'PaperPositionMode','auto')
    print(H,'-dpng','-r0',[final_cem_path img_files(c).name(1:end-4) '.png']);
    write_cem_fixed_edge_id([final_cem_path img_files(c).name(1:end-4) '.cem'], top_cfrags, edges, h, w, merged_cf_idx(1:min(top_K, length(merged_cem))))
%     det_save_cemv([final_cem_path img_files(c).name(1:end-4) '.cemv'], top_cfrags);
    end
    close all;
end
