...
%% set paths
base_src_path = '/media/guoy/Research/Datasets/3D_Drawing/amsterdam-house-mcs-work/';
edge_src_path = [base_src_path 'SE_edges/'];
cfrags_dst_path = [base_src_path 'SE_MSEL/'];
...
for c = 3%1:length(img_files)
    ...
    cmd = ['!../util/dborl_compute_curve_frags_greedy ' input ' ' output];
    eval(cmd);
...
%         p = 1 / (1 + exp(-([1, bg_grad, sat_grad, hue_grad, abs_k, edge_sparsity, wigg, len, mean_conf]-fmean_2)*beta_2'));
        p = contour_length_mex(merged_cem{i}');
        P_vec = [P_vec p];
    end 
...
%     top_cfrags =  merged_cem(1:min(top_K, length(merged_cem)));
    top_cfrags =  merged_cem(P_vec>prune_len);
