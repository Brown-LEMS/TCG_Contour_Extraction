...
{
    double wZ = 0; // weight for zero crossing
    double wG = 0.7; // weight for gradient magnitude
    ...
    while(!L.empty()){
        ...
        for (int dx = -1; dx <=1; dx++){
            ...
            for(int dy= -1; dy<=1; dy++){
                ...
                double cost_temp = q_node->cost * len_mat[qx*h+qy] + step_cost(q_node, r_node, E_map, E_map_soft, O_map, h, w, s);
                //std::cout << "cost_temp" << cost_temp << std::endl;
                double len_temp = len_mat[qx*h+qy] + sqrt(dx*dx + dy*dy);
                cost_temp = cost_temp/len_temp;
