function [ TO_edgemap, TO_orientation, edginfo ] = subpix_TO_correction( edgemap,theta,thresh,sigma, n)
if(nargin<5)
    n=0;
end
...
[h,w] = size(edgemap);
if(n>0)
    edgemap = imresize(edgemap, [(h-1)*2^n+1, (w-1)*2^n+1]);
    theta = imresize(theta, [(h-1)*2^n+1, (w-1)*2^n+1]);
    margin = margin*2^n;
end
...
edginfo = [subpix_x'/(n+1) subpix_y'/(n+1) atan2(subpix_dir_y2', subpix_dir_x2') mag_e']; 
TO_edgemap = zeros([h,w]);
TO_orientation = zeros([h,w]);
