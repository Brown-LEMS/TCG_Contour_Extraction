function [edges_trans, cfrags_trans] = transform_Contours_given_GT_depth(ID1,ID2, cfrags_idx1, edges1, datasetFilePath)
...
% z_ind = find((max(z_vec, [], 2)-min(z_vec, [], 2))>5);     
% z(z_ind) = min(z_vec(z_ind,:), [], 2);
z = min(z_vec, [], 2);
