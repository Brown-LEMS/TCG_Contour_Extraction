function [new_cfrags, new_cfrags_idx, T_junctions] = contour_introduce_jct(cfrags, cfrags_idx, h, w, edges)
% locate the conners and break the contour
search_nbr_range = 5; % search range from an end of curve fragment 
jct_nbr_range = 5; % supression range of an existing junction
search_ori_range = pi/9; % search angle at an end point of cfrag
...
ref_table_jct_pts = zeros([h, w]); % save the vid
...
for vid = 1:length(G_test.var) 
    ...
    %%%% check end points in the neighborehood
    x_min = max(1, xx-search_nbr_range);
    x_max = min(w, xx+search_nbr_range);
    y_min = max(1, yy-search_nbr_range);
    y_max = min(h, yy+search_nbr_range);
    nbr_end_vids = unique(free_end_map(y_min:y_max, x_min:x_max));
    nbr_end_vids(nbr_end_vids==0) = [];
    nbr_end_vids(nbr_end_vids==vid) = [];
    if(isempty(nbr_end_vids))
       continue; 
    end
    ...
    if(is_starting_point(vid))
        dx = cur_c(min(e_size, 4),1) - cur_c(1,1);
        dy = cur_c(min(e_size, 4),2) - cur_c(1,2);
    else
        dx = cur_c(max(e_size-3, 1) ,1) - cur_c(end,1);
        dy = cur_c(max(e_size-3, 1),2) - cur_c(end,2);        
    end
    exist_dx = dx;
    exist_dy = dy;
    qualify_end_vids = [];
    ...
    if(isempty(qualify_end_vids))
       continue; 
    end
    
    
     % locate gaps to fill between straight lines, need to be super smooth
    if(length(qualify_end_vids)==1 && cos_vec(1) > 0.8)      
         cid = G_test.var(qualify_end_vids).nbrs_fac;
...
%     Y_junctions = [Y_junctions; cur_edge];
    ref_table_jct_pts(y_min:y_max, x_min:x_max) = vid;
...
%     end
    ...
    if(c_len>(10) && length(cur_c_idx) > c_len_th+1)
        ...
        for j = 1:length(unique_start_id)
            ...
            if(length(cfrags_idx{c_id}) < 2*approach_clen_th)
                continue;
            end
...
%                 Y_junctions = [Y_junctions; cur_c(1, :)];
                % update ref maps, remove c_id
...
%                 Y_junctions = [Y_junctions; cur_c(end, :)];
                % update ref maps, remove c_id
                ref_table_start_pts(ref_table_start_pts==c_id) = 0;
                continue;
            end
...
%             end
            ...
            T_junctions = [T_junctions; cur_c(e_id, :)];
%             introduced_num_junction_points = introduced_num_junction_points + 1; 
            cur_break_e_ids =  [cur_break_e_ids e_id];
            prev_e_id = e_id;
            % update ref maps, remove c_id
...
%             end
            ...
            cos_ori_diff = abs(c_ori_vec*a_ori_vec'/norm(c_ori_vec)/norm(a_ori_vec));
            if(cos_ori_diff > cos(pi/9))% || cos_ori_diff < cos(pi/3))
                continue;
            end
...
%             introduced_num_junction_points = introduced_num_junction_points + 1; 
        ...
        %% make a decision on breaking: based on the smoothness at the attaching point of the original contour
