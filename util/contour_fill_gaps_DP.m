function [cfrags, cfrags_idx, edges] = contour_fill_gaps_DP(cfrags, cfrags_idx, h, w, edges, edgemap_soft, O_map, params)
% Last Modified: 02/27/2018
% 1. Treat DP augmented curves as additional curves, in case to remove
% those free-end low-conf ones later
% 2. perceptual completion can just extend existing curves
r_range = params.DP_gap_range;
theta_range = params.DP_angle_th;
contrast_th = params.DP_contrast_th;
...
%     end
    ...
    [cost_mat, back_p_mat, len_mat] = DP_gap_cpt(edgemap_ref, edgemap_soft(y_min:y_max, x_min:x_max), ...
                                            O_map(y_min:y_max, x_min:x_max), start_point_ref, h_ref, w_ref, theta_range, contrast_th);
    ...
    %%%%%%%%%% add an additional contour
    if(is_start) % DP started from the start edge of a contour
        ...
        cfrag_new = [add_edges; cfrags{cid}(1,:)];
        if(replace_end_edge_idx )
            add_edges(1,:) = [];
            cfrag_new(1,:) = edges(replace_end_edge_idx,:);
        end
        cfrags = [cfrags cfrag_new];
        edges = [edges; add_edges];
        cur_last_eid = size(edges, 1);
        cfrag_idx_new = [prev_last_eid+1:cur_last_eid cfrags_idx{cid}(1)];
        eid2update = prev_last_eid+1;
        if(replace_end_edge_idx)
            cfrag_idx_new = [replace_end_edge_idx cfrag_idx_new];
%             eid2update = replace_end_edge_idx;
            eid2update = 0;
        end
        cfrags_idx = [cfrags_idx cfrag_idx_new];
        prev_last_eid = cur_last_eid;
    else % DP started from the end edge of a contour
        ...
        cfrag_new = [cfrags{cid}(end,:); add_edges];      
        
        if(replace_end_edge_idx)
            add_edges(end,:) = [];
            cfrag_new(end,:) = edges(replace_end_edge_idx,:);
        end
        cfrags = [cfrags cfrag_new];
        edges = [edges; add_edges];
        cur_last_eid = size(edges, 1);
        cfrag_idx_new = [cfrags_idx{cid}(end) prev_last_eid+1:cur_last_eid];
        eid2update = cur_last_eid;
        if(replace_end_edge_idx)
            cfrag_idx_new = [cfrag_idx_new replace_end_edge_idx];
%             eid2update = replace_end_edge_idx;
            eid2update = 0;
        end
        cfrags_idx = [cfrags_idx cfrag_idx_new];
    ...
    EdgeGroupMap(sub2ind([h,w], round(opt_path_y), round(opt_path_x))) =  length(cfrags);
...
%     end
    ...
    add_end_edge_idx2 = 0;
    % only consider half range for T-junction
    line_end_x = cur_x + cos(angle)*search_nbr_range*0.75;
    line_end_y = cur_y + sin(angle)*search_nbr_range*0.75;
    line_X = linspace(cur_x, line_end_x, search_nbr_range);
    line_Y = linspace(cur_y, line_end_y, search_nbr_range);
    line_X = max(line_X, 1);
    line_X = min(line_X, w);
